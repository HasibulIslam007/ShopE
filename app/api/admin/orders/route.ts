...
export async function GET() { const guard = await requireAdmin(); if ("response" in guard) return guard.response; try { await connectDB(); return NextResponse.json({ orders: await Order.find({}).sort({ createdAt: -1 }).lean() }); } catch { return apiError("Unable to load orders", 500); } }
